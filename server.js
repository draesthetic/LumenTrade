...
const next = require(require.resolve('next', { paths: [path.join(__dirname, 'landing')] }));
...
const nextApp = next({ dev, dir: path.join(__dirname, 'landing') });
...
nextApp.prepare().then(() => {
  ...
  app.use('/app', express.static(path.join(__dirname, 'public'), { index: 'index.html' }));
  ...
  app.listen(PORT, () => {
    ...
    console.log(`Trade analytics app on http://localhost:${PORT}/app`);
  });
});
